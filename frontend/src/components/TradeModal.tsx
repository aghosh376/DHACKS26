...
import type { StockState, Portfolio } from "@/lib/simulator";
...
interface Props {
  ...
  stock: StockState;
  portfolio: Portfolio;
  onBuy: (shares: number) => boolean;
  onSell: (shares: number) => boolean;
  onClose: () => void;
}
...
const TradeModal = ({ professor, stock, portfolio, onBuy, onSell, onClose }: Props) => {
  ...
  const handleTrade = () => {
    const success = mode === "buy" ? onBuy(shares) : onSell(shares);
    if (success) {
